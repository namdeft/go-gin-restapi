...
func (suite *Suite) TestAddFavourite() {
	...
	suite.mock.ExpectQuery(`SELECT (.+) FROM "favourite" WHERE "user_id" = (.+) AND "dish_id" = (.+)`).
		WithArgs(userId, dishId).
		WillReturnRows(sqlmock.NewRows([]string{"user_id", "dish_id"}))

	suite.mock.ExpectExec(`INSERT INTO "favourite" (.+) VALUES (.+)`).
		WithArgs(userId, dishId).
		WillReturnResult(sqlmock.NewResult(1, 1))

	err := suite.repo.AddFavourite(context.Background(), userId, dishId)
...
func (suite *Suite) TestGetFavourites() {
	...
	dishes, err := suite.repo.GetFavourites(context.Background(), userId)
	assert.Nil(suite.T(), err)
...
func (suite *Suite) TestDeleteFavourite() {
	...
	err := suite.repo.DeleteFavourite(context.Background(), userId, dishId)
	assert.Nil(suite.T(), err)
}
